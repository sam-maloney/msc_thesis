...
for i = 1:length(N)
    ...
    interp_ref = interp1(ref_rho(:,1), ref_rho(:,2), rho(:,1), 'pchip');
...
l_err = log(err_1);
order = (l_err(2:end)-l_err(1:end-1))./(l_dx(2:end)-l_dx(1:end-1));
semilogx(dx2,order,'--.','MarkerSize',mSize);

l_err = log(err_Inf);
order = (l_err(2:end)-l_err(1:end-1))./(l_dx(2:end)-l_dx(1:end-1));
semilogx(dx2,order,'--d');
